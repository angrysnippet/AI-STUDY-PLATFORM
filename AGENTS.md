...
server/                         Node + Express + TypeScript (CommonJS, tsx dev, tsc build)
  src/
    ...
    routes/agent.ts             POST /start, POST /message, GET /plan/:id
    routes/plans.ts             list/get/delete plans + get/POST progress (sequential gating)
...
- **M3 (next):** auth + multi-user — JWT + Google OAuth with a `/api/auth/dev-login` stub;
  replace the `dev-user` constant in `routes/plans.ts` (`currentUser()`) with `req.user.id`.
- **M4:** deploy (Vercel + Railway/Render + Atlas) + credential setup guide.
